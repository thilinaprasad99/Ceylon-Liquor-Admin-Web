...
const DeleteWines = ({data,handleClose,handleSuccess}) => {
  ...
  const [isLoading, setIsLoading] = useState(false);
    ...
    const deleteWine = async (id) => {
        setIsLoading(true);
        axios
          .delete(`http://150.136.140.225:3700/api/public/Wines/delete/${id}`)
          .then((response) => {
    ...
    return (
        ...
        <DialogActions>
          ...
          autoFocus>
              {isLoading ? "Loading..." : "Delete Wines"}
          </Button>
        </DialogActions>
        </div>
    )
