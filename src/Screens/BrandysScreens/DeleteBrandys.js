...
const DeleteBrandys = ({data,handleClose,handleSuccess}) => {
  ...
  const [isLoading, setIsLoading] = useState(false);
    ...
    const deleteBrandy = async (id) => {
        setIsLoading(true);
        axios
          .delete(`http://150.136.140.225:3700/api/public/Brandys/delete/${id}`)
          .then((response) => {
    ...
    return (
        ...
        <DialogActions>
          ...
          <Button variant="contained"
              ...
              disabled={isLoading}
          onClick={() => {
            deleteBrandy(data.id);
          }}
          autoFocus>
              {isLoading ? "Loading..." : "Delete Brandys"}
          </Button>
        </DialogActions>
        </div>
    )
