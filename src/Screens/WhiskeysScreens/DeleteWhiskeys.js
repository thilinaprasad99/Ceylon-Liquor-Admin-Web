...
const DeleteWhiskeys = ({data,handleClose,handleSuccess}) => {
  ...
  const [isLoading, setIsLoading] = useState(false);
    ...
    const deleteWhiskey = async (id) => {
        setIsLoading(true);
        axios
          .delete(`http://150.136.140.225:3700/api/public/Whiskeys/delete/${id}`)
          .then((response) => {
    ...
    return (
        ...
        <DialogActions>
          ...
          <Button variant="contained"
              ...
              disabled={isLoading}
          onClick={() => {
            deleteWhiskey(data.id);
          }}
          autoFocus>
              {isLoading ? "Loading..." : "Delete Whiskeys"}
          </Button>
        </DialogActions>
        </div>
    )
